#include <iostream>  // hacker rank question
#include <vector>    
using namespace std;
...
void Unoin(vector<int> &parent,vector<int> &sz,vector<int>&minimal, vector<int> & maximal, vector<int> &rank, int a, int b){
    a = find(parent, a);
    b = find(parent, b);

    if(a == b) return; // both a and b are same
    if(rank[a] > rank[b]){
        rank[a]++;
        parent[b] = a;
        sz[a] += sz[b];
...
{ // note time complexity is (log*n);
    ...
    while (m--)
    {
        ...
        if(str == "union"){
            int x,y;
            cin>>x>>y;
            Unoin(parent,sz,minimal,maximal, rank, x, y);
        }else{
            int x;
            cin>>x;
            x = find(parent, x);
